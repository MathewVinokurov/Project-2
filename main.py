...
pygame.display.set_caption('Фуфик в бегах')
...
def start_screen(fon_filename):
    ...
    while True:
        ...
        print_text(["Фуфик в бегах"], 200, 500)
        pygame.display.flip()
        clock.tick(FPS)
    ...
    while do:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                x, y = event.pos
                if 10 <= x < 170 and height - 50 <= y < height - 30:  # Main menu
                    do = False
                    return 0
                elif width // 2 <= x < width // 2 + 235 and height - 50 <= y < height - 30:  # Restart level
                    do = False
                    return 1
                elif width // 4 <= x < width // 4 + 215 and height - 100 <= y < height - 80:  # Next level
                    do = False
                    return 2
        screen.fill((204, 204, 204))
        # print_text(
        #     ["Поздравляем!", f"Вы победили на уровне {cur + 1}"], 50, 50, color="white", text_size=60)
        print_text(
            array, 50, 50, color="white", text_size=60)
        pygame.draw.rect(screen, pygame.Color(
            "red"), (10, height - 50, 160, 20))
        print_text(["Основное меню"], 12, height - 50)
        pygame.draw.rect(screen, pygame.Color("green"),
                         (width // 2, height - 50, 235, 20))
        print_text(["Перезапустить уровень"], width // 2, height - 50)
        pygame.draw.rect(screen, pygame.Color("blue"),
                         (width // 4, height - 100, 215, 20))
        print_text(["Следующий уровень"], width // 4, height - 100)
        clock.tick(FPS)
        pygame.display.flip()


def last_level_end(cur):
    global running
    do = True
    while do:
        for event in pygame.event.get():
        ...
        screen.fill((204, 204, 204))
        print_text(
            ["Поздравляем!", "Вы победили на ", "последнем уровне"], 50, 50, color="white", text_size=60)
        pygame.draw.rect(screen, pygame.Color(
            "red"), (10, height - 50, 160, 20))
        print_text(["Основное меню"], 12, height - 50)
        pygame.draw.rect(screen, pygame.Color("green"),
...
def lose_end(cur):
    ...
    while do:
        ...
        screen.fill((204, 204, 204))
        print_text(
            ["К сожалению, ", "Вам не удалось пройти ", "данный уровень"], 50, 50, color="white", text_size=60)
        pygame.draw.rect(screen, pygame.Color(
            "red"), (10, height - 50, 160, 20))
        print_text(["Основное меню"], 12, height - 50)
        pygame.draw.rect(screen, pygame.Color("green"),
...
def play_mode2():
    global empty_group, wall_group, lemon_group, player, mode2_res, MODE, running
    cur = 0
    while cur < len(list_levels):
        levelname = list_levels[cur]
        ...
        player = Player(player_x, player_y)
        boom = None
        playing, is_stop, ok = True, False, False
        restart = False
        t, n = 60000, 0
        while playing:
            for event in pygame.event.get():
            ...
            if playing and is_stop:
                ...
                t -= clock.tick(FPS)
                print_text(
                    ["Текущий уровень:", f"номер {cur + 1}"], 10, height - 50, color="white")
                print_text([f"Осталось времени: {round(t / 1000, 1)} сек, набрано: {n}, ",
                            f"осталось набрать : {N - n}"], width // 3, 10, color="white")
                pygame.display.flip()
                if t < 0:
        ...
        if restart:
            continue
        if ok:
            mode2_res = max(mode2_res, cur + 1)
            if cur < len(list_levels) - 1:
                # _event_ = <0 -- main menu, 1 -- restart level, 2 -- next level>
                _event_ = win_end(
                    cur, ["Поздравляем!", f"Вы победили на уровне {cur + 1}"])
                if _event_ == 0:
                    MODE = 0
                    break
                elif _event_ == 1:
                ...
                # _event_ = <0 -- main menu, 1 -- restart level>
                _event_ = last_level_end(cur)
                if _event_ == 0:
                    MODE = 0
                    break
                elif _event_ == 1:
            ...
            if cur >= mode2_res:
                ...
                # _event_ = <0 -- main menu, 1 -- restart level, 2 -- next level>
                _event_ = win_end(cur, ["К сожалению, ", "Вам не удалось пройти ",
                                        "данный уровень сейчас", "но Вы прошли ", "его раннее!"])
                if _event_ == 0:
                    MODE = 0
                    break
                elif _event_ == 1:
...
if __name__ == '__main__':
    ...
    while running:
        ...
        if MODE == 1:
            ...
            print_text(["Оформите нашу подписку \"плюс\"",
                        "чтобы открыть этот режим"], 50, 50, surface=Sub)
